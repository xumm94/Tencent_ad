...
def one_hot_feature_process(feature):
    global data
    print("------------------processing" + " " + feature +"----------")
    m_s = set()
    for m in data[feature].unique():
        m = m.split(' ')
        for e in m:
            m_s.add(e)
    cato = list(m_s)
    for i in range(len(cato)):
        cato[i] = feature + cato[i]
    rows = data.shape[0]
    cols = len(cato)
    fin = pd.DataFrame(np.zeros((rows, cols), dtype= int), columns = cato)
    for index in tqdm(range(data.shape[0])):
        Status = data.loc[index, feature].split(' ')
        for i in range(len(Status)):
            Status[i] = feature + Status[i]
        fin.loc[index, Status] = 1
...
def feature_pre_process(one_hot_features, vector_features):
    ...
    for feature in one_hot_features:
        new_features.append(p.apply_async(one_hot_feature_process, args=(feature,)))
    p.close()
    p.join()
    for feature in one_hot_features:
    	del data[feature]
...
if __name__ == '__main__':
    ...
    len_valid = 1500000  # set size of cross-validation set
...
##################################################
    ...
    if not os.path.exists(path + 'onehot_processed_data.csv'):
        print('loading train...')
...
#          feature engineering                   #
##################################################
    print('adding count feature of aid...')
    adCount = data['aid'].value_counts()
    adCount = pd.DataFrame({'aid': adCount.index, 'adCount': adCount.values})
    data = data.merge(adCount, on='aid', how='left')
